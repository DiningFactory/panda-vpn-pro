...
👇 **机场购前须知**：
 ...
 - 协议：目前大部分机场所使用的协议有 `Shadowsocks`，`Trojan`，`VMess`，这三种**常用协议在主流的代理软件上都可得到支持**，所以，**使用了常用协议的机场我不作标识**。而 `VLess` 和 `Hysteria2` 这两种**新兴协议则只有少量的代理软件支持**。对于中转机场来说，目前最好用的协议依然是 `Shadowsocks` 协议，几乎所有中转机场都是使用的此协议。至于直连机场，由于线路结构的原因，使用此协议会很快被墙，所以几乎所有直连机场都不使用此协议。至于前面所提到的两个新协议，其目前基本只用于直连机场，被墙的几率从一定程度上也会低于 `Troajan` 和 `VMess`，因为 GFW 对于新协议流量特征的掌握程度肯定少与另外两者，但这其实只是从理论层面上分析，并非说明新协议 100% 优于旧协议。若需使用到这两种新协议，Windows 和 Mac 系统可使用 [Clash Verge](https://github.com/zzzgydi/clash-verge/releases/tag/v1.3.8)（版本不可低于此链接的版本，即 `v1.3.8`，否则将只能使用 `VLess`，无法使用 `Hyteria2`。且要将内核切换为 `Meta` 内核。），Android 系统可使用 [Clash Meta For Android](https://github.com/MetaCubeX/ClashMetaForAndroid/releases/tag/v2.9.0)（版本不可低于此链接的版本，即 `v2.9.0`， 否则将只能使用 `VLess`，无法使用 `Hysteria2`），ios 系统可使用 `Shadowrocket`（版本不可低于 `v2.2.35`，否则的话将只能使用 `VLess`，而无法使用 `Hysteria2`）。当然，还有其他代理软件也支持这两种协议，此处就不一一列举了，感兴趣的自行查阅资料。针对这部分**使用了新兴协议的机场**，**我在机场官网后面加上了相关标识**。标识含义如下：
   1. `V`：表示该机场大部分节点都为常用协议，极少部分节点使用了 `VLess` 协议。
   2. `H`：表示该机场大部分节点都为常用协议，极少部分节点使用了 `Hysteria2` 协议。
   3. `*V`：表示该机场所有的节点都为 `VLess` 协议。
   4. `*H`：表示该机场所有的节点都为 `Hysteria2` 协议。
   5. `*VH`：表示该机场所有的节点都为 `VLess` 或 `Hysteria2` 协议。
...
| 2023.05.05 | [OvO](https://ovofast.com/#/register?code=0pI5OPXj)<br>`*VH` | 直连 | 1 | 5.00 / 600 GB | None | 72 hours | None |
