...
- 🗓️ 2026.07.05 🗓️ `鱼鱼` 的用户请注意，该机场目前大部分节点都处于不可用的状态，原因未知，暂无太大跑路风险，等待修复即可。此通知将于恢复后删除。🗓️ 2026.07.06 🗓️ 目前已恢复，但又未完全恢复（少了许多节点），暂不建议购买，有待继续观察。🗓️ 2026.07.08 🗓️ 所有节点再次不可用。🗓️ 2026.07.08 🗓️ 再次恢复（依旧少了许多节点）。另外，个人认为机场主近期对机场的维护变得过于佛系了，因此暂停推荐（观察一段时间后会考虑重新推荐）。此通知以及该机场的链接将于 7.20 删除。🗓️ 2026.07.10 🗓️ 目前大部分节点再次处于不可用状态，此前从未出现近期这种表现，具有较高跑路风险，强烈不建议再续费。🗓️ 2026.07.13 🗓️ 再再再次恢复，跑路风险基本解除，但短期内依旧不作推荐。
- 🗓️ 2026.07.06 🗓️ 昨日，`Xray`内核的开发团队宣布停止维护中国翻墙生态，只针对俄罗斯和伊朗进行维护，[详见此](https://t.me/s/projectXtls/3373)。个人观点（注：以下所说的停止维护指的是停止针对中国用户翻墙需求的维护，相关内核和协议依然是会更新的）：① 受影响的主要是使用了 `Xray` 内核的代理软件以及使用了 `Vless`，`XHTTP`，`Reality` 协议的机场（这几个协议和 `Xray` 内核均出自同一开发团队）。② `Xray` 的停止维护对大部分用户无影响，因为使用 `Xray` 的用户数量是远不及 `Mihomo` 的（当然，这并不代表 `Xray` 不好，只不过 `Mihomo` 的 GUI 普遍颜值高，更容易受用户青睐）。即使是使用 `Xray` 的客户端，短期内也照样能用。就如当初的 CFW 删库，依然能用很久。哪怕是现在，你硬要用也可以，只不过存在兼容性差以及安全性低等缺点。③ `Vless` 的停止维护对大部分用户更是没有影响，因为节点用什么协议是机场主需要考虑的事情，用户只需保证自己所使用的代理软件能够支持机场所使用的协议即可。实际上，自 4 月份的拔线事件后，市面上使用 `Vless` 的机场已经少很多了，大部分都去使用 `AnyTLS` 或封端了。不过，这并不能说明 `AnyTLS` 就比 `Vless` 更好。④ 至于 `XHTTP`，目前使用该协议的机场极其少，它的停止维护对用户的影响就更是微乎其微了。⑤ `Reality` 一般会搭配 `Vless` 或 `XHTTP` 进行使用，相关影响参考这两个协议即可。⑥ `Xray` 的开发团队与其他开发者以及用户的爱恨情仇，感兴趣的自行了解，本人无资格做任何评价。此通知将于 8.1 移至“次要通知”。
- 🗓️ 2026.07.10 🗓️ `Doria` 的用户请注意，该机场目前大部分节点都处于不可用或断流的状态，暂无跑路风险。虽没有跑路风险，但近期整体体验较差，因此暂停推荐（观察一段时间后会考虑重新推荐）。此通知以及该机场的链接将于 7.20 删除。🗓️ 2026.07.17 🗓️ 现已恢复，但只剩下少量节点，短期内依然不作推荐。
- 🗓️ 2026.07.17 🗓️ `壹速云` 的用户请注意，该机场目前大部分节点都处于不可用的状态，原因未知，暂无跑路风险，等待修复即可。此通知将于恢复后删除。
...
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 2019.08.15 | 🟨<br>[壹速云](https://www.onesy1.cc/auth/register?code=R4Lm)<br>`*A` | 中转 | 1 | 19.90 / 100 GB | 188.00 / 1000 GB | Null | Null |
...
| ~~2023.04.15~~ | 🟨<br>[~~鱼鱼~~](https://252288.xyz/#/register?invite_code=lw92ZJPe)<br>~~`*AX`~~<br>~~🔥🔥~~<br>~~📜~~ | ~~直连~~<br>+ | ~~1~~ | ~~7.99 / 128 GB~~ | ~~20.00 / 100 GB~~ | ~~Null~~ | ~~Null~~ |
| 2023.05.01 | [农夫山泉](https://qqq.nfsqttt.com/#/register?code=HvoPMFli)<br>🔒 | 中转 | 1 | 15.00 / 200 GB | 45.00 / 200 GB | Null | Null |
| 2023.07.01 | [蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>📜 | 中转 | 1 | 20.00 / 200 GB | Null | Null | 5.5 折<br>`FWY3`<br>所有套餐可用<br>7.25 前有效<br><br>充 200 送 30<br>充 500 送 100<br>充 1000 送 300<br>7.25 前有效<br><br>7 折<br>`IEPLerror`<br>所有套餐可用<br>长期有效<br><br>9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
...
| ~~2024.07.12~~ | 🟨<br>[~~Doriya~~](https://rtx.al/#/register?code=hAlv337j)<br>~~`A`~~ | ~~中转~~ | ~~0.5 & 0.7~~ | ~~8.00 / 100 GB~~ | ~~Null~~ | ~~Null~~ | ~~Null~~ |  
| 2024.07.18 | [FlyBit](https://flybit.vip/#/register?code=HpHWTZX1)<br>📜 | 中转 | 1 | 15.00 / 128 GB | 36.00 / 128 GB | 2 GB | Null |
...
- FlyBit：
     - 该机场将用户分成 A，B 两组，分别对应不同的线路。B 组线路比 A 组好，理论上体验更好，不过用户默认都在 A 组。机场主每天会根据“特定条件”将部分用户从 A 组移至 B 组。用户也可手动提交工单申请进入 B 组，只要满足特定条件，机场主便会同意。至于这个所谓的特定条件是什么就不得而知了，因为这是机场主用来区分“内鬼”的，以尽可能减少通报或者拔线等情况，自然是不会公开判定标准的。
