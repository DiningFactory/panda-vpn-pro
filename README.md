...
📢 **重要通知**：此部分仅用于对于机场的重要变动及突发情况作出必要的说明。
 ...
 - **使用 `OvO` 的用户请注意**：（2023.11.1）此机场所有节点不再继续使用 `Trojan` 协议，更改为 `vless` + `hysteria2` 协议。对于用户的影响是，部分代理软件将无法继续使用此机场，如 `Clash for Windows`。Windows 系统可使用 `v2rayN`，Android 系统可使用 `v2rayNG`，ios 系统可使用 `Shadowrocket`。详细内容可在该机场官网查看。此机场是有试用的且机场官网本身有发出相关公告，所以这个通知其实不是很必要。但考虑到此变动影响较大，所以我依然专门说明一下。此通知将于 11.15 删除。
...
| 2022.05.10 | [SPCloud](https://aff.spcloud.vip#/register?code=zQUEVPsX) | 中转 | 1 | 10.00 / 100 GB | 15.00 / 50 GB | None | 7 折<br>`理性消费从我做起`<br>所有套餐可用<br>短期有效 |
