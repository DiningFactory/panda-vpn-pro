...
- 🗓️ 2026.04.03 🗓️ `蜂窝云` 的用户请注意，该机场目前已关闭订阅链接（此前还可通过发送工单获取），只能使用官方客户端，我已为其添加 🔒 标识。不过，之前已经获取过订阅链接的，也还能用（少部分人可能不行），至于能用多久就不知道了。后续如果能恢复手动获取，会另行通知。此通知将于 4.15 删除。🗓️ 2026.04.03 🗓️ 已恢复正常订阅，但是节点都已变成直连节点。我已将 🔒 标识移除，并将机场类型改为“直连”。之后能否恢复中转不得而知。
