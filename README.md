...
- 🗓️ 2026.04.17 🗓️ 自即日起，不再对使用了 `Vless` 和 `Hysteria2` 协议的机场作标识。大致原因如下：① 这两个协议如今已算不上新协议，绝大部分主流代理软件都已支持，所以没必要专门标识。② 由于近期的“拔线事件”，不少中转机场上线了直连节点，甚至会用上 `AnyTLS` 这类更为新的协议，所以“旧标识”要为“新标识”让道（两者虽可共存，但前者确实没必要存在了）。详见机场列表上方的 `协议 2.0`。此通知将于 5.1 删除。
