...
- 🗓️ 2026.04.02 🗓️ 如上方通知所说，拔线文件可能是假的，但内容确实是真的。这两天机场圈相当动荡，大部分中转机场受此影响，体验大幅度下滑，节点大面积不可用也是屡见不鲜。因此，**至少未来一个月内**，**都强烈不建议购买或续费中转机场**，**只建议购买直连机场**。此通知暂不删除（在此通知存在期间，中转机场如有不稳定的情况，我不会为其添加 🟨 标识或通知，当前情况，默认全都如此。除非有跑路迹象，或是改为强制使用客户端等特殊情况，才会专门通知）。
- 🗓️ 2026.04.03 🗓️ `蜂窝云` 的用户请注意，该机场目前已关闭订阅链接（此前还可通过发送工单获取），只能使用官方客户端，我已为其添加 🔒 标识。不过，之前已经获取过订阅链接的，也还能用（少部分人可能不行），至于能用多久就不知道了。后续如果能恢复手动获取，会另行通知。此通知将于 4.15 删除。🗓️ 2026.04.03 🗓️ 已恢复正常订阅，但是节点这两天会改成直连节点。我已将 🔒 标识移除，并将机场类型改为“直连”。之后能否恢复中转不得而知。
...
| 2020.04.22 | [次元雲](https://ciyy.one/#/register?code=tsL8Me6h)<br>`V` | 中转 | 1 | 17.00 / 256 GB | 90.00 / 520 GB | Null | Null |
...
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5) | 直连 | 0.5 & 1 | 20.00 / 200 GB | Null | Null | 8 折<br>`QM2026`<br>所有套餐可用<br>4.13 前有效<br><br>9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
