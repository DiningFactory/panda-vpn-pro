...
- 🗓️ 2025.05.01 🗓️ 不知不觉，`Clash for Windows` 删库事件已过去一年半了，这期间时不时就有人爆出各种漏洞，从安全角度考虑，强烈不建议大家继续使用这类没人维护的代理软件了，包括 `Clash Verge` 等。顺带一提，PC 代理软件 `Nekoray` 的作者在前段时间表示由于精力有限，不再维护该软件了，已将该项目归档。只会继续维护 `NekoBox`。不过作者也表示，如果 `Nekoray` 存在重大 BUG，也可能会继续维护。此通知将于 6.1 移至 `次要通知` 中。
