...
- 🗓️ 2025.11.24 🗓️ `M78星云` 的用户请注意，该机场近期新增了 `Vless` 协议的直连节点（我已在机场列表添加相应标识）用于防失联，但是一些过于老旧的代理软件会由于不支持该协议而无法导入订阅（比如 `Clash for Windows`），如你所使用的代理软件不支持该协议，建议尽快更换为其他代理软件。此通知将于 12.15 删除。
