...
- 🗓️ 2026.04.03 🗓️ `蜂窝云` 的用户请注意，该机场目前已关闭订阅链接（此前还可通过发送工单获取），只能使用官方客户端，我已为其添加 🔒 标识。不过，之前已经获取过订阅链接的，也还能用（少部分人可能不行），至于能用多久就不知道了。后续如果能恢复手动获取，会另行通知。此通知将于 4.15 删除。
...
| 2023.07.01 | [蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>🔒 | 中转 | 0.5 & 1 | 20.00 / 200 GB | Null | Null | 9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
