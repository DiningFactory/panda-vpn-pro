...
- 🗓️ 2025.07.10 🗓️ `蜂窝云` 的用户请注意，该机场现在已新增入口为 `IPv6` 的节点。拥有 `IPv6` 的用户可通过以下方式使用：进入官网 --> 节点列表 --> 订阅节点筛选 --> 将“选择入口类型”的“通用入口”改为“IPv6入口” --> 保存更改 --> 前往代理软件更新订阅。🗓️ 2025.07.23 🗓️ 目前已不需要使用 `IPv6` 入口，`IPv6` 入口反而可能不稳定。之前如果有使用 `IPv6` 入口，建议切回 `IPv4`。即在“订阅节点筛选”中将“IPv6入口”改回“通用入口”。
- 🗓️ 2025.07.10 🗓️ `兔兔云` 的用户请注意，该机场现在已将原先 `SS` 协议的节点更改为仅 `IPv6` 可用，并新增大量 `Vless` 节点供 `IPv4` 用户使用。注意，部分老旧代理软件不支持 `Vless` 节点（如果客户端太老获取不到 `Vless` 节点，才会获取到 `SS` 节点），如 `Clash for Windows`。当然，不是说“较新”的代理软件就一定支持 `Vless`，比如 `Sufboard` 和 `Surge` 就一直不支持。请及时更换为支持 `Vless` 协议的代理软件。
- 🗓️ 2025.07.11 🗓️ `云安云` 的用户请注意，该机场所有节点已支持 `IPv6`。🗓️ 2025.07.20 🗓️ 该机场已将订阅链接改为 5 分钟的“阅后即焚”模式（详见上方通知）。
- 🗓️ 2025.07.16 🗓️ `次元云` 的用户请注意，该机场所有节点均已支持解析为 `IPv6`。🗓️ 2025.08.17 🗓️ 目前已不支持 `IPv6`，原因未知。9.1 前若依旧不支持，此条通知以及 7.16 的通知将删除。
- 🗓️ 2025.07.22 🗓️ `M78星云` 的用户请注意，该机场目前一小部分节点已支持且仅支持 `IPv6`，这部分节点名称带有相应标识。
- 🗓️ 2025.08.05 🗓️ `极速云` 的用户请注意，该机场已将订阅链接改为 5 分钟的“阅后即焚”模式（详见上方通知）。
- 🗓️ 2025.08.15 🗓️ `农夫山泉` 的用户请注意，该机场几乎已将所有节点替换为 `Vless` 协议，请尽快更换为支持该协议的代理软件。
...
- 🗓️ 2025.08.17 🗓️ `FlyBit` 的用户请注意，该机场表示由于近期入口通报，体验下降，已通过增加个人中心余额的方式进行补偿，不同套餐的补偿情况不一，具体补偿方案请前往官网查看（如未收到补偿，可发工单询问是否遗漏）。另外，因怀疑订阅泄漏，已重置所有用户的订阅链接，因此需要前往官网重新获取。此通知将于 9.1 删除。
...
- 🗓️ 2025.08.20 🗓️ 目前 GFW 对大量（也可能是所有）国外 `443` 端口的流量进行了屏蔽（双向屏蔽，即“内外”几乎无法互相访问），`ipv6` 暂时几乎没影响。比如无法在不挂梯子且无 `ipv6` 的情况下访问苹果的 [官网](https://www.apple.com/) 。有说是为了 9.3 阅兵做准备，也有说是配置失误等，真实原因未知。对机场的影响主要是无法在不挂代理的情况访问官网或是更新订阅。🗓️ 2025.08.20 🗓️ 该情况大约发生于凌晨十二点半，而我大约是在一点半才知道，刚发完此通知就基本恢复了，尬住了。不过还是有待继续观察，此通知将于 9.5 删除。
- 🗓️ 2025.08.27 🗓️ `次元云` 的用户请注意，该机场近期稳定性较差，会不定时断流或是持续一定时间的节点几乎全红，尤其是晚上。虽无跑路风险，但暂不推荐购买或续费。此通知将于机场恢复稳定且持续稳定一段时间后删除。
- 🗓️ 2025.08.29 🗓️ `Mihomo` 内核表示由于其下游之一的 `Mihomo Party` 不接受改名，可能会采取停止更新的做法。`Mihomo` 内核原名为 `Clash Meta` 内核，本身基于 `Clash` 内核，在 `Clash for Windows` 删库事件后改名并继续维护。而 `Mihomo Party` 则是使用了 `Mihomo` 内核的 GUI 代理软件。然后，`Mihomo Party` 代理软件的官方主页置顶了某机场的链接，且有小道消息传出 `Mihomo Party` 这个代理软件被相应机场收购了。而由于 `Mihomo Party` 这个代理软件的名称中带有 `Mihomo` 字样，会让一些用户误以为 `Mihomo` 内核与 `Mihomo Party` 代理软件是同一个开发者，从而对 `Mihomo` 内核的安全性等问题产生质疑，即让 `Mihomo` 内核的风评受害。另外，这种误会还可能会对 `Mihomo` 内核团队产生一系列不可预知的风险。因此，`Mihomo` 内核的开发者近期在开源协议中明确要求任何非官方下游项目的名称不得包含 `Mihomo` 相关的字样（其实也不是近期，早在今年 2 月份时就已经提出来了。具体可查看[此更新](https://github.com/MetaCubeX/mihomo/commit/ccc3f84da22dad5210ce2f7986c34198cc75a3dc)），然而 `Mihomo Party` 代理软件的开发者依旧违反该协议，一直不接受改名。因此，`Mihomo` 内核的开发者表示可能会停止维护该项目以避免 `Mihomo Party` 代理软件给他们所带来的潜在风险。
...
| 2019.08.15 | [壹速云](https://www.onesy1.cc/auth/register?code=R4Lm) | 中转 | 1 | 10.90 / 150 GB | 188.00 / 1000 GB | Null | 8 折<br>`onesy6thAnniversary`<br>所有套餐可用<br>9.1 前有效 |
| 2020.04.22 | [次元雲](https://ciyy.me/#/register?code=tsL8Me6h) | 中转 | 1 | 10.00 / 128 GB | 85.00 / 500 GB | Null | 52 / 不限时 / 520 GB<br>短期内可购买 |
| 2022.11.06 | [赔钱机场](https://xn--mes358aby2apfg.com/#/register?code=e3c70bPe)<br>`*VH` | 直连 | 0.01 & 1 | 2.99 / 500 GB | 18.90 / 1000 GB | Null | 75 折<br>`七夕`<br>所有套餐可用<br>9.1 前有效 |
| 2022.12.07 | [一分机场](https://xn--4gqx1hgtfdmt.com/#/register?code=ziP4woeh)<br>`*VH` | 直连 | 0.1 & 1 | 2.00 / 100 GB | 19.88 / 1000 GB | Null | 85 折<br>`85off`<br>所有套餐可用<br>9.1 前有效 |
| 2023.02.03 | [自由猫](https://us.freecat.cloud/register?code=8S3V1vsr) | 中转 | 1 | 9.00 / 100 GB | 50.00 / 500 GB | Null | 7 折<br>`2025QiXi`<br>所有套餐可用<br>9.1 前有效<br><br>8 折<br>`FREECAT`<br>所有套餐可用<br>长期有效 |
...
| 2023.09.12 | [M78星云](https://m78star.cloud/#/register?code=7IWr2dOP) | 中转 | 1 |  7.80 / 80 GB | 88.00 / 400 GB | Null | 85 折<br>`season85`<br>季付套餐可用<br>82 折<br>`halfyear82`<br>半年付套餐可用<br>8 折<br>`year80`<br>年付套餐可用<br>9.7 前有效 |
...
| 2024.08.14 | [喵喵云](https://dash.nyanss20250507.lat/register?code=dfsKFF7s) | 中转 | 0.3 & 1 | 7.50 / 50 GB | 25.00 / 100 GB | Null | Null |
...
| 机场 | 入口运营商及地区 |
| :---: | :---: |
| SAC | 电信：江苏盐城<br>移动：江苏盐城 |
| 壹速云 | 电信：广东东莞<br>联通：广东广州<br>移动：广东广州，江苏徐州 |
| 次元云 | 电信：广东东莞，福建泉州<br>移动：广东广州，江苏徐州 |
| 自由猫 | 移动：广东深圳，广东佛山 |
| 极速云 | 电信：广东深圳，广东广州<br>联通：广东深圳，广东东莞<br>移动：广东深圳，广东东莞 |
| 农夫山泉 | 电信：广东东莞<br>联通：广东东莞<br>移动：广东揭阳 |
| 蜂窝云 | 电信：广东东莞 |
| Metcha | 电信：广东广州<br>联通：广东东莞<br>移动：广东深圳，广东江门，上海 |
| M78星云 | 移动：广东佛山，广东广州，湖北武汉 |
| 云安云 | 移动：广东深圳 |
| 兔兔云 | 电信：云南昆明<br>移动：广东广州 |
| FlyBit | 电信：广东东莞<br>联通：广东广州<br>移动：广东广州 |
| 喵喵云 | 电信：湖北十堰<br>移动：广东深圳<br>腾讯云：广东广州，北京，上海 |
...
8. 将代理软件升级到最新版本或更换其他代理软件。注意，平常使用过程中在没遇到问题时是不需要更新代理软件的（除非代理软件出现某种漏洞），**代理软件在稳不在新**。虽说不用追求最新版本，**但也尽量不要使用版本过旧的代理软件**。有些过旧的代理软件虽然能用，但不一定好用，比如可能造成节点频繁掉线。
