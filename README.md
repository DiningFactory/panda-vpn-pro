...
- 🗓️ 2026.03.15 🗓️ `蜂窝云` 的用户请注意，该机场目前断流频繁，原因未知，无需担心，等待修复即可。此通知将于恢复后删除。
- 🗓️ 2026.03.15 🗓️ `FlyBit` 的用户请注意，该机场目前约一半节点处于不可用的状态，原因未知，无需担心，等待修复即可。此通知将于恢复后删除。
...
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>🔥<br>📜<br>`V` | 中转 | 0.5 & 1 | 20.00 / 200 GB | 40.00 / 210 GB | Null | 9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
...
| 2024.07.18 | 🟨<br>[FlyBit](https://flybit.vip/#/register?code=HpHWTZX1) | 中转 | 1 | 15.00 / 128 GB | 36.00 / 128 GB | 2 GB | 9 折<br>`flybit`<br>所有套餐可用<br>长期有效 |
| 2024.08.14 | [NyanSS](https://dash.nyanss20250507.lat/register?code=dfsKFF7s) | 中转 | 0.5 & 1 | 7.50 / 50 GB | 44.90 / 200 GB | Null | Null |
| 2025.03.07 | [ofoNET](https://ofonet.net/#/register?code=q3kx6Xt9)<br>🔥🔑 | 中转 | 1 | 9.90 / 200 GB | 55.88 / 384 GB | Null | 8 折<br>`ofonet`<br>所有套餐可用<br>3.16 前有效 |
...
- 蜂窝云：可对订阅链接所获取的节点进行一点个性化设置，步骤为：官网 --> 节点列表 --> 订阅节点筛选。在该页面中可对节点进行简单的筛选，默认获取的是全部节点，入口为 `IPv4`。有两个注意点：① 如果你修改为“自定义获取节点”，那么建议定期进入该页面检查节点情况。因为，假如后来机场主新添加了一些节点，那你导入节点时，是不会获取到这些节点的，需要手动在该页面中勾选它们。② 在该页面中可以将入口由默认的 `IPv4`（页面中的“通用入口”）改为 `IPv6`，但除非特殊需求，否则不建议更改。
