...
- 蜂窝云：可对订阅链接所获取的节点进行一点个性化设置，步骤为：官网 --> 节点列表 --> 订阅节点筛选。在该页面中可对节点进行简单的筛选，默认获取的是全部节点，入口为 `IPv4`。有两个注意点：① 如果你修改为“自定义获取节点”，那么建议定期进入该页面检查节点情况。因为，假如后来机场主新添加了一些节点，那你导入节点时，是不会获取到这些节点的，需要手动在该页面中勾选它们。② 在该页面中还可以将入口由默认的 `IPv4`（页面中的“通用入口”）改为 `IPv6`，但除非特殊需求，否则不建议更改。
