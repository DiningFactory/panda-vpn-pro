...
- 🗓️ 2026.04.03 🗓️ `蜂窝云` 的用户请注意，该机场目前已关闭订阅链接（此前还可通过发送工单获取），只能使用官方客户端，我已为其添加 🔒 标识。不过，之前已经获取过订阅链接的，也还能用（少部分人可能不行），至于能用多久就不知道了。后续如果能恢复手动获取，会另行通知。此通知将于 4.15 删除。🗓️ 2026.04.03 🗓️ 已恢复正常订阅，但是节点这两天会改成直连节点。我已将 🔒 标识移除，并将机场类型改为“直连”。之后能否恢复中转不得而知。🗓️ 2026.04.09 🗓️ 截至目前其中转节点仍然保留着，考虑到近期我是不推荐中转机场的，并且其直连节点当前阶段的体验在我看来，是不如本就为直连机场的节点的，因此已将其类型重新修改为“中转”。
...
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5) | 中转 | 0.5 & 1 | 20.00 / 200 GB | Null | Null | 8 折<br>`QM2026`<br>所有套餐可用<br>4.13 前有效<br><br>9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
