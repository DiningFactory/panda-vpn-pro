...
- 🗓️ 2025.07.10 🗓️ `次元云` 的用户请注意，该机场部分节点已支持 `IPv6`，依然支持解析为 `IPv4`，但 `IPv6` 体验更佳。拥有 `IPv6` 的用户无需手动设置，只需保证设备和代理软件没有禁用相关功能即可。非 `IPv6` 的移动用户暂勿购买该机场，大概率会完全无法使用。此通知暂定 9.1 删除。
