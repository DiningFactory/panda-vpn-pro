...
- 🗓️ 2025.09.13 🗓️ 前几天刚通知说近期已经很少有入口被通报了，于是就把置顶长达两个月的通报事件的通知移至“大事留档”。然而好景不长，最近通报又多了起来，也是被狠狠打脸了。既然防火墙大哥的脾气如此阴晴不定，那我们只能**常态化看待通报**了。但我依然觉得不用过度担忧，总有应对方法的，比如未来可能会有越来越多的机场使用“阅后即焚”模式（在正文的“机场列表”上方有介绍）。此通知将长期保留。🗓️ 2025.09.28 🗓️ **有些机场虽然未采用阅后即焚模式**，**但是在受到通报后**，**机场主是有可能会重置所有用户的订阅链接的**。**因此**，**如果你发现所有节点都不可用**，**且订阅无法更新**，**请重新到官网导入订阅**。🗓️ 2025.11.04 🗓️ 随着通报的常态化，市面上有少部分机场已经采取了比“阅后即焚”更为极端的防护措施，那就是“取消订阅链接，只允许使用机场官方客户端”，即类似于传统 VPN 软件那样的登录后“一键连”的方式。本文的机场目前暂无采用该方式，但也建议做好相应心理准备。🗓️ 2025.12.06 🗓️ 从上个月底开始，在通报频繁的当下，又不断有机场的节点域名被污染，这个基本也算是常态化了（将代理软件的本地 DNS 设置为 `223.5.5.5` 或 `119.29.29.29` 等公共 DNS 能一定程度防止污染。如果代理软件不支持修改 DNS，那就直接改相应设备的 DNS。如果不知道怎么修改 DNS，请自行搜索 `代理软件 or 操作系统 or 手机品牌 + 怎么修改 DNS`）。入口通报只会影响到中转和专线，DNS 污染则连直连也会受影响。虽然依然不至于多恐慌，但可以看见的是，现在各机场的稳定性都是大不如前的，包括一线机场。总之，现在翻墙的 Debuff 越来越多，机场主的运营压力也是越来越大，压力大就容易撂挑子不干。因此，真心强烈建议不要年付！🗓️ 2026.02.08 🗓️ 有消息称国内多家 IDC 将对不合规的流量转发，宽带转售等业务进行清退，简而言之会使中转机场受到不小的打击。该消息的讨论热度不低，虽然暂未得到证实，但大概率不假。结合此前的通知以及 1.30 公安部发布的《网络犯罪防治法（征求意见稿）》来看，现在对翻墙的打击力度确实是越来越强的。客观上来讲，各机场的跑路风险也是越来越大的。所以呢，还是那句话，机场只推荐月付。至于“路会不会被彻底堵死”这种担忧，我觉得是没必要的，只要没有发生某种极其大的事件，是不可能这样的。此部分通知不是为了制造焦虑，而是想让大家不要冲动消费。
- 🗓️ 2026.02.08 🗓️ `ofoNET` 的用户请注意，该机场近期已全面将节点的协议从原先的 `Shadowsocks`（简称 `SS`） 更改为 `Shadowsocks-2022`（简称 `SS2022`，可理解为 `SS` 的升级版），从名字能看出这其实并不是一种非常新的协议，所以对大部分人来说是没影响的。除了已删库的 `Clash for Windows` 以及支持协议一直较少的 `Surfboard` 外（使用这些软件会无法导入订阅或者导入后是“空的”），只要版本不是太久远的代理软件，都是支持该协议的。另外，该协议在大部分代理软件中依然显示为 `Shadowsocks`，这无关紧要，只要导入订阅后能看到许多节点就没问题（**该协议的加密依赖于系统时间**，**如果超时的话请校准系统时间与北京时间**，**误差尽量控制在 30 秒内**）。鉴于市面上使用 `SS2022` 的机场不多，所以我暂不在机场列表中作专门的标识，只在此处单独说明。且考虑到 `Clash for Windows` 时至今日依然有不少用户（强烈建议这些用户更换其他代理软件），所以长期保留该通知。
...
👇 **机场购前须知**：
 ...
 - 🔥：表示该机场的**订阅链接**使用了“**阅后即焚**”模式：指在官网复制订阅链接后，该订阅链接在一定时间内（一般为 5 分钟）可导入代理软件。超过该时间后，如果想导入代理软件，需要重新复制订阅链接（注意，这个时间不一定是从你复制那一刻算起的，不同机场的实现方式会有一定差异。如果复制后导入失败，那可能是正巧失效了，此时可以尝试刷新网页，再重新复制订阅链接）。因此，**导入订阅后**，**请勿再更新订阅**（因为有些代理软件在更新订阅时会先清除当前订阅的节点，而由于这种订阅是阅后即焚的，就会导致更新后节点全部消失，需要重新前往官网获取），**如果设置了自动更新订阅**，**也请关闭**（虽然我在之前的通知里说近期要勤更新订阅，但是“阅后即焚”模式是相反的，导入后就别再更新）。所谓“阅后即焚”模式是在 2025 年 7 月的“入口通报事件”中，为了防止订阅泄漏以至于入口被通报，由某一线机场率先使用，因为有着不错的效果（注意：**有效果不代表可以完全避免被通报**），所以有一些机场也在跟进该模式。但该模式也是有缺点的，那就是当节点发生变化时，需要重新到官网获取订阅链接，比较麻烦（**想保证使用最新节点可每天获取一次订阅。但其实也没必要这样，节点变化一般不会很频繁，等到节点不可用或体验不佳时再重新订阅即可**）。以及无法通过更新订阅来查看自己当前剩余流量，只能前往官网查看。如果无法接受这些缺点，请避开带有 🔥 标识的机场。
    ...
    - 🔥🔑：与上述“阅后即焚”的区别在于，它**使用自家机场的节点时**，**可随意更新订阅**。比如 A 机场带有该标识，那么在超过阅后即焚的时间限制后，如果不使用代理（未连接任何节点）或者使用的是 B 机场的节点，将无法再更新订阅，如需更新订阅，需重新前往 A 机场的官网获取订阅。而如果使用的是 A 机场的节点，那么绝大多数时候都可以更新订阅（**例外的情况是**：**机场主重置了所有人的订阅链接**），即相当于没有“阅后即焚”的限制。
...
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 2019.08.15 | [壹速云](https://www.onesy1.cc/auth/register?code=R4Lm) | 中转 | 1 | 10.90 / 150 GB | 188.00 / 1000 GB | Null | 8 折<br>`ONESY2026`<br>所有套餐可用<br>2.24 前有效 |
| 2020.04.22 | [次元雲](https://ciyy.one/#/register?code=tsL8Me6h) | 中转 | 1 | 10.00 / 128 GB | 90.00 / 520 GB | Null | 7.8 折<br>`2026-78`<br>部分年付套餐可用<br>8 折<br>`2026-888`<br>所有套餐可用<br>2.28 前有效 |
| 2022.09.01 | [Metcha](https://ww1.638242.xyz/#/register?code=MzTl1fEj) | 中转 | 1 | 9.88 / 128 GB | 258.88 / 1024 GB | Null | Null |
| 2022.11.06 | [赔钱机场](https://xn--mes358aby2apfg.com/#/register?code=e3c70bPe)<br>`*VH` | 直连 | 0.01 & 1 | 2.99 / 500 GB | 18.90 / 1000 GB | Null | 7 折<br>`新年快乐`<br>所有套餐可用<br>短期有效 |
...
| 2023.02.03 | [自由猫](https://us.freecat.cloud/register?code=8S3V1vsr) | 中转 | 1 | 9.00 / 100 GB | 50.00 / 500 GB | Null | 6.6 折<br>`2026NewYear`<br>所有套餐可用<br>2.24 前有效<br><br>8 折<br>`FREECAT`<br>所有套餐可用<br>限用一次<br>长期有效 |
...
| 2023.09.12 | [M78星云](https://m78star.cloud/#/register?code=7IWr2dOP)<br>🔥<br>`V` | 中转 | 1 |  7.80 / 80 GB | 99.00 / 400 GB | Null | 85 折<br>`season85`<br>季付套餐可用<br>82 折<br>`halfyear82`<br>半年付套餐可用<br>8 折<br>`year80`<br>年付套餐可用<br>2.28 前有效 |
| 2024.03.01 | [兔兔云](https://www.tutuyun.uk/auth/register?code=QKEtXiLs)<br>`*V` | 中转 | 1 | 11.88 / 140 GB | 18.00 / 60 GB | 12 hours | Null |
| 2024.07.12 | [Doriya](https://dash.rtx.al/#/register?code=hAlv337j)<br>`V` | 中转 | 1 | 8.00 / 100 GB | Null | Null | 8.8 折<br>`劝君今夕不须眠。且满满，泛觥船。大家沉醉对芳筵。愿新年，胜旧年。`<br>所有套餐可用<br>2.26 前有效 |  
...
| 2025.03.07 | [ofoNET](https://ofonet.net/#/register?code=q3kx6Xt9)<br>🔥🔑 | 中转 | 1 | 9.90 / 200 GB | 55.88 / 384 GB | Null | 8 折<br>`ofonet2026`<br>所有套餐可用<br>2.22 前有效 |
| 2025.03.14 | [iNetS](https://inets.io/#/register?code=YVUGJDrv)<br>`*V` | 直连<br>+ | 0.5 & 1 | 6.00 / 50 GB | Null | 7 days<br>极少节点 | 85 折<br>`MdAoVhjp`<br>所有套餐可用<br>3.3 前有效 |
| 2025.03.18 | [良心云](https://xn--9kqz23b19z.com/#/register?code=61Ysg3dm)<br>`*VH` | 直连 | 1 | 2.00 / 100 GB | 21.00 / 1000 GB | Null | 7 折<br>`LXY`<br>大部分套餐可用<br>2.21 前有效 |
...
| :---: | :---: |
| 壹速云 | 联通：广东东莞 |
| 次元雲 | 电信：广东东莞，广东佛山<br>移动：广东广州，广东深圳 |
| Metcha | 电信：广东广州，广东东莞<br>联通：广东东莞，广东佛山<br>移动：广东广州，广东深圳 |
| 自由猫 | 移动：广东江门 |
| 农夫山泉 | 电信：广东东莞<br>联通：广东东莞<br>移动：广东揭阳 |
| 蜂窝云 | 电信：广东东莞<br>联通：广东广州<br>移动：广东深圳 |
| M78星云 | 电信：广东广州<br>联通：广东东莞<br>移动：广东广州，广东江门 |
| 兔兔云 | 移动：广东深圳，吉林，四川 |
| Doriya | 联通：广东深圳<br>移动：广东深圳，广东广州 |
| FlyBit | 电信：广东深圳<br>联通：广东深圳<br>移动：广东广州 |
| NyanSS | 移动：广东深圳 |
| ofoNet | 电信：广东广州<br>移动：广东广州 |
...
| Mac | `Clash for Windows`，`ClashX`，`ClashX Pro`，`Clash Verge`，`Clash Verge Rev`，`Clash Nyanpasu`，`FlClash`，`V2rayU`，`sing-box`，`Stash`，`Surge`，`Quantumult X`，`Hiddify`，`Qv2ray`，`Streisand`，`Clash Party`，`Karing`，`Clash Mi`，`Sparkle`，`ClashMac` |
