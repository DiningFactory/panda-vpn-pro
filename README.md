...
- 🗓️ 2025.07.10 🗓️ `次元云` 的用户请注意，该机场部分节点已支持 `IPv6`，依然支持解析为 `IPv4`，但 `IPv6` 体验更佳。拥有 `IPv6` 的用户无需手动设置，更新订阅后，只需保证设备和代理软件没有禁用相关功能即可。非 `IPv6` 的移动用户暂勿购买该机场，大概率会完全无法使用。此通知暂定 9.1 删除。
- 🗓️ 2025.07.10 🗓️ `山水云` 的用户请注意，该机场所有节点基本全部处于不可用的状态，虽然在近期这种现象属于“正常现象”，但并未发现机场主有维护的迹象，具有一定的跑路风险，暂不建议购买或续费。🗓️ 2025.07.10 🗓️ 目前节点已基本恢复。跑路风险暂时解除。
...
| 2025.03.14 | [iNetS](https://inets.io/#/register?code=DpHxThbX)<br>`*V` | 直连<br>+ | 0.2 & 1 | 4.90 / 50 GB | 13.90 / 100 GB | 7 days<br>极少节点 | Null | 
