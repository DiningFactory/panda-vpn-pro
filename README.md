...
📢 **重要通知**：此部分仅用于对于机场的重要变动及突发情况作出必要的说明。
 ...
 - （2023.11.2）Clash for Windows 已停止更新，原因未知，但不影响使用。若有顾虑，可更换其他代理软件。注意，`Clash For Windows` 与 `Clash Core` 是没有任何关系的，也就是说，它停止更新不影响 `Clash` 的任何分支。（2023.11.3）`Clash Core`，`Clash.Meta Core` 也停止更新，原因未知。带来的影响是，其相关的分支在内核方面都将不再更新，但是，目前依然不影响使用，有顾虑者，可选择其他内核的代理软件。
