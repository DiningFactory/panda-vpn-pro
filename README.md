...
📢 **重要通知**：此部分仅用于对于机场的重要变动及突发情况作出必要的说明。
 - **使用 `次元雲` 的用户请注意**：🗓️ 2023.10.29 🗓️ ~~此机场近期部分节点（`香港F`，`香港G`，`香港I`，`新加坡E`，`美国C`，`印度`，`挪威`，`澳大利亚`，`阿根廷`，`土耳其`）的后端出现问题，导致流量统计异常，因此，如果你发现了你的流量发生了异常（例如莫名其妙少了很多），那么可以在机场官网发送工单等待处理（发送工单的步骤：进入官网 --> 点击“我的工单” --> 点击“新的工单”按钮 --> 填写相关信息），处理之后会重置流量。~~ 🗓️ 2023.10.31 🗓️ 由于机场面板发生了 BUG，导致大部分用户流量异常，机场主现已重置所有用户的流量进行补偿（重置流量的意思是：将已用流量归 0），因此，29 号的通知不必理会了。不过，这个 BUG 目前暂未彻底修复，需要等机场主慢慢排查原因，因此依旧可能会出现流量异常的情况。不过不必过于担心，等待修复与补偿即可。🗓️ 2023.11.1 🗓️ 机场已恢复正常，不过，11.5 会再重置一次流量。11.15 前如果不再出现 BUG，此通知将会删除。
 - **使用 `OvO` 的用户请注意**：🗓️ 2023.11.1 🗓️ 此机场所有节点不再继续使用 `Trojan` 协议，更改为 `Vless` + `Hysteria2` 协议。对于用户的影响是，部分代理软件将无法继续使用此机场，如 `Clash For Windows`。Windows 和 Mac 系统可使用 [Clash Verge](https://github.com/zzzgydi/clash-verge/releases/tag/v1.3.8)（注意：必须使用此链接的版本，即 `v1.3.8`，若低于此版本，则只能使用 `Vless`，无法使用 `Hysteria2`。且要将内核切换为 `Meta` 内核。），Android 系统可使用 [Clash Meta For Android](https://github.com/MetaCubeX/ClashMetaForAndroid/releases/tag/v2.9.0)（注意：必须使用此链接的版本，即 `v2.9.0`， 请勿使用 Play Store，因为其目前尚未上架该版本。若低于此版本，则只能使用 `Vless`，无法使用 `Hysteria2`），ios 系统可使用 `Shadowrocket`（注意，版本不可低于 `v2.2.35`，否则的话将只能使用 `Vless`，而无法使用 `Hysteria2`）。考虑到此变动对用户影响较大，为了防止用户在使用过程发现“节点全红”或者“无法导入订阅”等情况时感到慌张，所以此通知将长期存在。后续我会在正文部分增加对机场协议的补充，届时会删除此通知。
 - 🗓️ 2023.11.1 🗓️ 近期 GFW 变得非常高，尤其今晚，大部分机场的官网，节点都遭重了，不用着急，耐心等待恢复即可。此通知将于大部分机场恢复且 GFW 不再增强时删除。
 - **使用 `雨燕云` 的用户请注意**：🗓️ 2023.11.1 🗓️ 此机场由于机场主的操作失误，导致部分用户的流量被意外清空，如有相关情况，可在官网发送工单恢复流量，且可得到一定的补偿。此通知将于 11.15 删除。
 - 🗓️ 2023.11.2 🗓️ `Clash for Windows` 已停止更新，原因未知，但不影响使用。若有顾虑，可更换其他代理软件。注意，`Clash For Windows` 与 `Clash Core` 是没有任何关系的，也就是说，它停止更新不影响 `Clash` 的任何分支。🗓️ 2023.11.3 🗓️ `Clash Core`，`Clash.Meta Core`，`Clash For Android`，`Clash Meta For Android`，`Clash Verge`，`Tuic`，`Helloworld 路由器插件`，`Gui.For.Clash`，`homeproxy`，`Fclash`，`ClashX` 也纷纷停止更新或删库，部分进行了 archived。另外，**忠心劝诫各位不要在墙内的任何地方谈论任何与翻墙相关的事情**，无论是为了自身安全还是为了代理工具的安全。近期本来只是 `Clash For Window` 删库，结果各种无知群众莫名把相关话题带上微博热搜，此后一堆 Core，GUI 删库，这可以说是这些无知人士带节奏所引起的一系列连锁反应。至于目前这些代理软件还能不能用呢，答案是能。只有到了这些代理工具或是内核被发现有严重漏洞那一天，才不可以使用。然而这丝毫不用感到惊慌。代理工具以及内核千千万万，除了 `Clash` 还有 `V2ray` 等，而且旧事物的褪去总会有新事物来代替，互联网的更新是日新月异的，这点我相信各位很清楚。总之，请勿在墙内大谈特谈相关事件，此通知将长期存在。
 - **使用 `COO` 的用户请注意**：🗓️ 2023.11.3 🗓️ 此机场的官网在维护时，导致 11.2 的数据丢失，在 11.2 所注册的用户可重新注册账户，然后提交工单，说明交易时间与套餐，之后等待等处即可，处理完会恢复套餐。此通知将于 11.7 删除。
