 ...
 ❗ ❗ ❗ ❗ ❗ ❗ ❗ ❗ ❗ ❗ ❗ ❗ 重要通知（2023.10.29）：**使用次元雲的用户请注意**，~~此机场近期部分节点（`香港F`，`香港G`，`香港I`，`新加坡E`，`美国C`，`印度`，`挪威`，`澳大利亚`，`阿根廷`，`土耳其`）的后端出现问题，导致流量统计异常，因此，如果你发现了你的流量发生了异常（例如莫名其妙少了很多），那么可以在机场官网发送工单等待处理（发送工单的步骤：进入官网 --> 点击“我的工单” --> 点击“新的工单”按钮 --> 填写相关信息），处理之后会重置流量。~~（2023.10.31）由于机场面板发生了 BUG，导致大部分用户流量异常，机场主现已重置所有用户的流量进行补偿（重置流量的意思是：将已用流量归 0），因此，29 号的通知不必理会了。不过，这个 BUG 目前暂未彻底修复，需要等机场主慢慢排查原因，因此依旧可能会出现流量异常的情况。不过不必过于担心，等待修复与补偿即可。
