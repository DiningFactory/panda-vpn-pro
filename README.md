...
👇 **机场购前须知**：
 ...
 - 🔒：**表示该机场只能使用其提供的客户端**（客户端的下载方式通常在官网的“使用文档”中有说明），**不能使用订阅链接导入到代理工具**，就类似于传统的 `VPN` 软件。这种方式的好处是能够较有效防止通报与攻击，不过也有不少坏处，如：部分设备可能不支持（如软路由，iOS，Linux。不同机场支持情况不同，具体支持情况请前往官网查看。购买这类机场前务必查看该机场是否支持自己的设备），客户端安全性未知（如果担心客户端会泄露个人隐私或是携带病毒等，那么就不要选择带有该标识的机场），可玩性较低（第三方代理工具一般能折腾的地方较多，而私有客户端往往功能较单一），切换机场麻烦（对于有多家机场的人来说，想在多家机场的节点中来回切换，原先只需要在一个软件中操作，而这种情况则需要在多个软件中操作）。另外，这类机场由于强制使用客户端，因此，其无论使用的协议多新多罕见，我都不提供标注（做标注的目的是防止大家使用的代理工具不支持相应协议），入口信息也不提供（准确说是我几乎无法获取，这也正是其防通报与攻击的原因。不过通过抓包的方式应该还是可以获取，但是会麻烦很多）。总而言之，这种方式是一把双刃剑，如果无法接受其诸多缺点，那么请直接避开带有这类标识的机场。但即使能接受，**购买套餐前**，**建议先下载相应的客户端**，**并确保能成功登录**，**再购买**，防止因各种原因无法使用客户端而“白买”。另外，如果发现所有节点都无法使用且长时间未恢复，那么可能是当前版本的客户端存在某些漏洞被“弃用”了，此时建议去机场官网下载新版本的客户端（客户端本身不一定支持更新功能）。
...
| 2025.03.07 | [ofoNET](https://ofotw.org/#/register?code=q3kx6Xt9)<br>🔥🔑 | 中转 | 1 | 9.90 / 200 GB | 55.88 / 384 GB | Null | 9 折<br>`ofonet9`<br>所有套餐可用<br>3.26 前有效 |
