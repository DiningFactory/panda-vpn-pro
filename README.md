...
---
📢 **重要通知**：此部分仅用于对于机场的一些重要变动及突发情况作出必要的说明。
 - （2023.10.29）**使用 `次元雲` 的用户请注意**，~~此机场近期部分节点（`香港F`，`香港G`，`香港I`，`新加坡E`，`美国C`，`印度`，`挪威`，`澳大利亚`，`阿根廷`，`土耳其`）的后端出现问题，导致流量统计异常，因此，如果你发现了你的流量发生了异常（例如莫名其妙少了很多），那么可以在机场官网发送工单等待处理（发送工单的步骤：进入官网 --> 点击“我的工单” --> 点击“新的工单”按钮 --> 填写相关信息），处理之后会重置流量。~~（2023.10.31）由于机场面板发生了 BUG，导致大部分用户流量异常，机场主现已重置所有用户的流量进行补偿（重置流量的意思是：将已用流量归 0），因此，29 号的通知不必理会了。不过，这个 BUG 目前暂未彻底修复，需要等机场主慢慢排查原因，因此依旧可能会出现流量异常的情况。不过不必过于担心，等待修复与补偿即可。（2023.11.1）机场已恢复正常。11.15 前如果不再出现 BUG，此通知将会删除。
 - （2023.11.1）：**使用 `OvO` 的用户请注意**，此机场所有节点不再继续使用 `Trojan` 协议，更改为 `vless` + `hysteria2` 协议。对于用户的影响是，部分代理软件将无法继续使用此机场，如 Clash for Windows。Windows 系统可使用 `Clash Verge`，Android 系统可使用 `Clash Meta`，ios 系统可使用 `Shadowrocket`。另外，此机场目前订阅功能还未完善，除 ios 外，需要手动在订阅后方加入 `&flag=meta `。详细内容可在该机场官网查看。此机场是有试用的且机场官网本身有发出相关公共，所以这个通知其实不是很必要。但考虑到此变动影响较大，所以我依然专门说明一下。此通知将在 11.15 删除。
---
