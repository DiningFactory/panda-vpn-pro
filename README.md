...
- 🗓️ 2026.04.22 🗓️ `壹速云` 的用户请注意，该机场近期会逐步将所有节点的协议更换为 `AnyTLS` 协议，请查当前所使用的代理软件是否支持该协议，如果不支持，建议尽快更新至支持该协议的版本或者更换为其他支持该协议的代理软件（可参考本文机场列表上方的 `协议 2.0`）。此通知将于 5.20 删除。🗓️ 2026.04.24 🗓️ 目前大部分节点都已更换完毕（注：需前往官网重新导入订阅，`Shadowrocket` 则直接更新订阅即可）。
