...
- 🗓️ 2026.07.10 🗓️ `Doria` 的用户请注意，该机场目前大部分节点都处于不可用或断流的状态，暂无跑路风险。虽没有跑路风险，但近期整体体验较差，因此暂停推荐（观察一段时间后会考虑重新推荐）。此通知以及该机场的链接将于 7.20 删除。
...
- FlyBit：
     - 该机场现在将用户分成 A，B 两组，分别对应不同的线路。B 组线路比 A 组好，理论上体验更好，不过用户默认都在 A 组。机场主每天会根据“特定条件”将部分用户从 A 组移至 B 组。用户也可手动提交工单申请进入 B 组，只要满足特定条件，机场主便会同意。至于这个所谓的特定条件是什么就不得而知了，因为这是机场主用来区分“内鬼”的，以尽可能减少通报或者拔线等情况，自然是不会公开判定标准的。
