...
- 🗓️ 2025.07.10 🗓️ `次元云` 的用户请注意，该机场部分节点已支持 `IPv6`，依然支持解析为 `IPv4`，但 `IPv6` 体验更佳，直接更新订阅即可。非 `IPv6` 的移动用户暂勿购买该机场，大概率会完全无法使用（即使是有 `IPv6` 的移动用户，也暂不建议购买。因为目前该机场的 `IPv6` 节点还较少）。此通知暂定 9.1 删除。🗓️ 2025.07.13 🗓️ 只有 `IPv4` 的移动用户已经可使用部分节点。🗓️ 2025.07.16 🗓️ 大部分节点已经恢复了（只有 `IPv4` 的移动用户大部分节点也可使用了）。
