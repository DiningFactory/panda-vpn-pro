...
- 🗓️ 2026.02.08 🗓️ `ofoNET` 的用户请注意，该机场近期已全面将节点的协议从原先的 `Shadowsocks`（简称 `SS`） 更改为 `Shadowsocks-2022`（简称 `SS2022`，可理解为 `SS` 的升级版），从名字能看出这其实并不是一种非常新的协议，所以对大部分人来说是没影响的。除了已删库的 `Clash for Windows` 以及支持协议一直较少的 `Surfboard` 外（使用这些软件会无法导入订阅或者导入后是“空的”），只要版本不是太久远的代理软件，都是支持该协议的。另外，该协议在大部分代理软件中依然显示为 `Shadowsocks`，这无关紧要，只要导入订阅后能看到许多节点就没问题（**该协议的加密依赖于系统时间**，**如果超时的话请校准系统时间与北京时间**，**误差尽量控制在 30 秒内**）。鉴于市面上使用 `SS2022` 的机场不多，所以我不在机场列表中作专门的标识，只在此处单独说明。且考虑到 `Clash for Windows` 时至今日依然有不少用户（强烈建议这些用户更换其他代理软件），所以长期保留该通知。
