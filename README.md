...
- 🗓️ 2025.07.10 🗓️ `蜂窝云` 的用户请注意，该机场现在已新增入口为 `IPv6` 的节点。拥有 `IPv6` 的用户可通过以下方式使用：进入官网 --> 节点列表 --> 订阅节点筛选 --> 将“选择入口类型”的“通用入口”改为“IPv6入口” --> 保存更改 --> 前往代理软件更新订阅。🗓️ 2025.07.22 🗓️ 该机场表示受本轮通报的影响，导致原先中转线路资源紧缺且体验不佳等问题，已经全部升级为 IEPL 专线，原先中转套餐的用户也同步升级（套餐有效期和流量不变）。然后，目前已停止出售中转套餐（个人猜测以后也许会重新出售），只出售专线套餐。
