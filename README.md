...
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>🔒<br>📜 | 中转 | 0.5 & 1 | 20.00 / 200 GB | Null | Null | 8 折<br>`QM2026`<br>所有套餐可用<br>4.13 前有效<br><br>9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
...
- 蜂窝云：
     ...
     - 在导入订阅链接时，如果你所使用的代理工具是 `V2rayN` 或 `V2rayNG`，那么，需要导入两个链接才能获取所有直连节点，即官网的“复制 V2ray 订阅”和“复制 V2ray-VLESS 订阅”。PC 版的 `v2rayN` 自带合并多个订阅链接的功能，首先，在“url”旁边的输入框填上其中一个订阅链接，之后，点击输入框旁边的“三个点”（版本如果极其老旧则没有这个功能，建议更新至新版本），会弹出一个新的输入框，在这个新的输入框中填上另一个链接即可（若想合并三个及以上的链接，则用“英文的逗号”隔开这些链接）。当然，也可以分别导入两个链接，当成两个“分组”。而移动端的 `V2rayNG` 截至目前的最新版本（`v2.0.18`）仍不支持该功能，只能分别导入。
