...
- 🗓️ 2025.06.03 🗓️ `iNets` 机场的用户请注意，如果发现订阅链接订阅失败的话，请将订阅链接 `https://miaomiao.inets.io/xxxxxx` 替换为 `https://miaomiao.myddns.me/xxxxxx`。若是问题依旧，可发工单联系机场主寻找其他解决方案或是退款（一般情况下，是会同意的），此通知将长期保留。
...
- 🗓️ 2025.07.10 🗓️ `次元云` 的用户请注意，该机场部分节点已支持 `IPv6`，依然支持解析为 `IPv4`，但 `IPv6` 体验更佳。拥有 `IPv6` 的用户无需手动设置，更新订阅后，只需保证设备和代理软件没有禁用相关功能即可。非 `IPv6` 的移动用户暂勿购买该机场，大概率会完全无法使用（即使是有 `IPv6` 的移动用户，也暂不建议购买。因为目前该机场的 `IPv6` 节点还较少）。此通知暂定 9.1 删除。
- 🗓️ 2025.07.11 🗓️ `云安云` 的用户请注意，该机场所有节点已支持 `IPv6`，拥有 `IPv6` 的用户无需手动设置，更新订阅后，只需保证设备和代理软件没有禁用相关功能即可。非 `IPv6` 的用户暂勿购买该机场，虽然依然支持解析 `IPv4`，但是大概率会完全无法使用。此通知暂定 9.1 删除。
