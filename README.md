...
- 🗓️ 2026.04.02 🗓️ 如上方通知所说，拔线文件可能是假的，但内容确实是真的。这两天机场圈相当动荡，大部分中转机场受此影响，体验大幅度下滑，节点大面积不可用也是屡见不鲜。因此，**至少未来一个月内**，**都强烈不建议购买或续费中转机场**，**只建议购买直连机场**。此通知暂不删除（在此通知存在期间，中转机场如有不稳定的情况，我不会为其添加 🟨 标识或通知，当前情况，默认全都如此。除非有跑路迹象，或是改为强制使用客户端等特殊情况，才会专门通知）。🗓️ 2026.04.09 🗓️ 部分中转机场近期已新增了直连节点以作应对，之后“拔线”如果力度加大或是持续时间较长，也许会有很多中转机场不得不转型为直连机场。但即使如此，在当前时期，我暂不会把他们的类型从“中转”改为“直连”。因为这个阶段我只推荐直连机场，而这些半路转型的，我不认为他们短期内能做得比原先就是搞直连的做得更好。🗓️ 2026.05.30 🗓️ GFW 这段时间暂无更进一步的动作，各机场的体验虽不及以前，但也逐渐趋向稳定（但马上临近敏感日期，未来一周内的稳定性难以避免地会再次下降）。6.20 前若无什么大事发生，此通知以及下方 5.22 的通知将删除。
- 🗓️ 2026.05.22 🗓️ 这段时间不少机场都使用了 `AnyTLS` 这种较新的协议试图破局，而在今天下午，很多使用了该协议的节点都被墙了。不知道是临近敏感日期，GFW 临时性加强，还是其他什么原因。此通知暂不删除。
- 🗓️ 2026.06.11 🗓️ `FlyBit` 的用户请注意，该机场现在将用户分成 A，B 两组，分别对应不同的线路。B 组线路比 A 组好，理论上体验更好，不过用户默认都在 A 组。机场主每天会根据“特定条件”将部分用户从 A 组移至 B 组。用户也可手动提交工单申请进入 B 组，只要满足特定条件，机场主便会同意。至于这个所谓的特定条件是什么就不得而知了，因为这是机场主用来区分“内鬼”的，以尽可能减少通报或者拔线等情况，自然是不会公开判定标准的。此通知将于 7.1 移至机场列表下方的“附加说明”中。
...
👇 **机场购前须知**：
 ...
 - 🔒：**表示该机场只能使用其提供的客户端**（客户端的下载方式通常在官网的“使用文档”中有说明），**不能使用订阅链接导入到代理工具**，就类似于传统的 `VPN` 软件。这种方式的好处是能够较有效防止通报与攻击，不过也有不少坏处，如：部分设备可能不支持（如软路由，iOS，Linux。不同机场支持情况不同，具体支持情况请前往官网查看。购买这类机场前务必查看该机场是否支持自己的设备），客户端安全性未知（如果担心客户端会泄露个人隐私或是携带病毒等，那么就不要选择带有该标识的机场），可玩性较低（第三方代理工具一般能折腾的地方较多，而私有客户端往往功能较单一），切换机场麻烦（对于有多家机场的人来说，想在多家机场的节点中来回切换，原先只需要在一个软件中操作，而这种情况则需要在多个软件中操作）。另外，这类机场由于强制使用客户端，因此，其无论使用的协议多新多罕见，我都不提供标注（做标注的目的是防止大家使用的代理工具不支持相应协议），入口信息也不提供（准确说是我几乎无法获取，这也正是其防通报与攻击的原因。不过通过抓包的方式其实还是可以获取，但是会麻烦很多）。总而言之，这种方式是一把双刃剑，如果无法接受其诸多缺点，那么请直接避开带有这类标识的机场。但即使能接受，**购买套餐前**，**建议先下载相应的客户端**，**并确保能成功登录**，**再购买**，防止因各种原因无法使用客户端而“白买”。另外，**如果发现所有节点都无法使用且长时间未恢复**，**那可能是当前版本的客户端因存在某些 BUG 而被“弃用”了**，**此时建议去机场官网下载新版本的客户端**（**客户端本身不一定支持更新功能**）。使用机场客户端时，个人建议将软件中所有 `IPv6` 相关的设置统统关闭，确实有需求才开启，这样能减少一些奇奇怪怪的问题。
...
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 2019.08.15 | [壹速云](https://www.onesy1.cc/auth/register?code=R4Lm)<br>`*A`<br>📜 | 中转 | 1 | 10.90 / 150 GB | 188.00 / 1000 GB | Null | Null |
| 2020.04.22 | [次元雲](https://ciyy.one/#/register?code=tsL8Me6h)<br>`A` | 中转 | 1 | 17.00 / 158 GB | Null | Null | Null |
| 2022.11.06 | [赔钱机场](https://xn--mes358aby2apfg.com/#/register?code=e3c70bPe) | 直连 | 0.01 & 1 | 2.99 / 500 GB | 18.90 / 1000 GB | Null | 75 折<br>`端午`<br>所有套餐可用<br>6.21 前有效 |
| 2022.12.07 | [一分机场](https://xn--4gqx1hgtfdmt.com/#/register?code=ziP4woeh) | 直连 | 0.1 & 1 | 2.00 / 100 GB | 19.88 / 1000 GB | Null | 85 折<br>`DW`<br>大部分套餐可用<br>6.22 前有效 |
...
| 2023.09.12 | [M78星云](https://m78star.cloud/#/register?code=7IWr2dOP)<br>🔒<br>📜 | 中转 | 1 |  12.80 / 150 GB | 99.00 / 300 GB | Null | 85 折<br>`season85`<br>季付套餐可用<br>82 折<br>`halfyear82`<br>半年付套餐可用<br>8 折<br>`year80`<br>年付套餐可用<br>6.24 前有效 |
