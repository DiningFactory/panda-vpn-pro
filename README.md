...
- **使用 `赔钱机场` 的用户请注意：** 🗓️ 2023.11.5 🗓️ 此机场已将大部分的协议从 Trojan 协议更改为 `VLess` 协议，若是你当前的代理软件不支持此协议，建议尽快更换。如：Windows，Mac --> `Clash Verge`，Android --> `Clash Meta For Android`，ios --> `shadowrocket`（小火箭）。此通知将于 11.15 删除。
