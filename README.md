...
- 🗓️ 2026.03.19 🗓️ `蜂窝云` 的用户请注意，该机场表示近期通报较严重，已暂停直接获取订阅链接（恢复时间未知），需通过发送工单的方式获取（或者使用该机场的客户端）。此通知暂不删除。🗓️ 2026.03.19 🗓️ 目前看来，该机场默认无法获取订阅链接的机制将长期存在，稍显麻烦，但也不是特别麻烦（只需要申请一次）。我已对该机场添加 ✋ 标识并在机场列表新增了对该标识的说明。此通知将于 4.1 删除。
- 🗓️ 2026.03.20 🗓️ `NyanSS` 的用户请注意，该机场目前大部分节点都处于不可用状态，原因未知，暂无太大跑路风险，无需过于担心，等待修复即可。此通知将于恢复后删除。
...
| 2023.07.01 | 🟨<br>[蜂窝云](https://api.fwcloud.life/auth/register?code=DZzGx5)<br>🔥<br>✋<br>📜<br>`V` | 中转 | 0.5 & 1 | 20.00 / 200 GB | 40.00 / 210 GB | Null | 9 折<br>`FW9`<br>所有套餐可用<br>长期有效 |
...
| 2024.08.14 | 🟨<br>[NyanSS](https://dash.nyanss20250507.lat/register?code=dfsKFF7s) | 中转 | 0.5 & 1 | 7.50 / 50 GB | 44.90 / 200 GB | Null | Null |
| 2025.03.07 | [ofoNET](https://ofonet.net/#/register?code=q3kx6Xt9)<br>🔥🔑 | 中转 | 1 | 9.90 / 200 GB | 55.88 / 384 GB | Null | Null |
| 2025.03.14 | [iNetS](https://inets.io/#/register?code=YVUGJDrv)<br>📜<br>`*V` | 直连<br>+ | 0.5 & 1 | 6.00 / 50 GB | Null | 7 days<br>极少节点 | Null |
| 2025.03.18 | [良心云](https://xn--9kqz23b19z.com/#/register?code=61Ysg3dm)<br>`*VH` | 直连 | 1 | 2.00 / 100 GB | 21.00 / 1000 GB | Null | Null |

📜 附加说明：

- 蜂窝云：
     - 该机场将订阅链接的获取方式改为“手动发工单申请”后，虽然只需要申请一次，审核通过之后，“获取订阅”的按钮就会重新出现，但它“阅后即焚”的机制是依然存在的。也就是说，虽然不需要每次获取订阅都发送工单，但仍需适应阅后即焚的使用方式（超过特定的时间后，无法更新订阅，需重新前往官网获取）。
     - 可对订阅链接所获取的节点进行一点个性化设置，步骤为：官网 --> 节点列表 --> 订阅节点筛选。在该页面中可对节点进行简单的筛选，默认获取的是全部节点，入口为 `IPv4`。有两个注意点：① 如果你修改为“自定义获取节点”，那么建议定期进入该页面检查节点情况。因为，假如后来机场主新添加了一些节点，那你导入节点时，是不会获取到这些节点的，需要手动在该页面中勾选它们。② 在该页面中可以将入口由默认的 `IPv4`（页面中的“通用入口”）改为 `IPv6`，但除非特殊需求，否则不建议更改。
- iNets：
     - 含极少量 `AnyTLS` 协议的节点，该协议较新，因此，支持该协议的代理软件还不太多（不过，对于绝大多数代理软件来说，即使订阅中存在本身不支持的协议，也不影响使用。只有少数过于老旧的代理软件，会因为订阅中存在本身不支持的协议，导致完全无法导入）。对该协议感兴趣的用户可使用支持该协议的代理软件。`Windows`，`Mac`，`Linux` 以及 `Android` 用户可使用 [FlClash](https://github.com/chen08209/FlClash/releases/tag/v0.8.77)（版本不可低于此链接的版本，即 `v0.8.77`）。`iOS` 可使用 `Shadowrocket`（版本不可低于 `v2.2.64`）。还有其他代理软件也支持该协议，就不都列举了，感兴趣的自行查阅资料。考虑到该机场目前只有极少量节点使用该协议，且该协议在机场里的普及度还很低，因此，我暂不会像 `Vless` 和 `Hysteria2` 协议一样专门为该协议作标识，只在此处说明。
...
| 次元雲 | 联通：广东东莞<br> 移动：广东广州，广东深圳 |
| Metcha | 电信：广东广州，广东东莞<br> 联通：广东东莞<br> 移动：广东广州，广东揭阳 |
| 自由猫 | 移动：广东广州 |
| 蜂窝云 | 电信：广东东莞<br> 联通：广东东莞<br> 移动：广东深圳 |
| M78星云 | 电信：广东广州<br> 联通：广东东莞<br> 移动：广东广州 |
| 兔兔云 | 电信：上海<br> 移动：广东广州，湖南长沙 |
| Doriya | 移动：广东深圳，广东广州，广东江门 |
| FlyBit | 电信：广东深圳 <br>联通：广东深圳 <br>移动：广东广州 |
| NyanSS | 电信：广东广州<br> 移动：广东广州 |
| ofoNet | 电信：广东广州 |
